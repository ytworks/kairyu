...
def _supported_swebench_platform(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda dataset, limit: tuple(
            f"instance-{index}" for index in range(1, (limit or 1) + 1)
        ),
        raising=False,
    )
...
def test_parse_swebench_report_accepts_official_completed_error_overlap():
    """SWE-bench 4.1 marks an invalid existing report completed and errored."""
    report = copy.deepcopy(SWEBENCH_REPORT)
    report["completed_instances"] = 3
    report["completed_ids"] = ["resolved-1", "unresolved-1", "error-1"]

    items, total = parse_swebench_report(
        report,
        selected_ids=(*report["submitted_ids"], "incomplete-1"),
    )
...
def test_swebench_selection_uses_official_dataset_order_before_limit(monkeypatch):
    monkeypatch.setattr(
        swe_mod,
        "load_hf_rows",
    ...
    assert load_selected_instance_ids("princeton-nlp/SWE-bench_Verified", 2) == (
        "z",
        "a",
    )
...
@pytest.mark.parametrize(
    "rows",
    [[], [{"instance_id": ""}], [{"instance_id": "a"}, {"instance_id": "a"}]],
)
def test_swebench_selection_rejects_ambiguous_denominators(monkeypatch, rows):
    monkeypatch.setattr(swe_mod, "load_hf_rows", lambda dataset, split: rows)

    with pytest.raises(ValueError, match="empty|invalid|duplicate"):
        load_selected_instance_ids("princeton-nlp/SWE-bench_Verified", None)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"a": {"instance_id": "a", "model_name_or_path": "m"}},
            id="missing-patch",
        ),
        pytest.param(
...
async def test_swebench_two_stage_flow_and_official_denominator(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        swe_mod,
        "load_selected_instance_ids",
        lambda dataset, limit: selected_ids,
        raising=False,
    )
    monkeypatch.setenv("SWEBENCH_TEST_KEY", "super-secret-key")
    ...
    def fake_run(command, capture_output, timeout, env, cwd, check):
        ...
        class Completed:
            returncode = 0
            stdout = b"stdout super-secret-key"
            stderr = b"stderr super-secret-key"
    ...
    artifacts = pair.methodology["artifacts"]
    assert Path(artifacts["root"]).is_dir()
    assert Path(artifacts["selection"]).is_file()
    assert Path(artifacts["predictions"]).is_file()
    assert Path(artifacts["official_report"]).is_file()
    for path in artifacts["stage_logs"].values():
...
async def test_swebench_missing_prediction_stays_in_official_denominator(
    ...
    def fake_run(command, capture_output, timeout, env, cwd, check):
        if "run_evaluation" not in " ".join(command):
            ...
            assert command[command.index("--instance_ids") + 1 :] == list(
                selected_ids
            )
            report = {
                "schema_version": 2,
                ...
                "empty_patch_instances": 0,
                "error_instances": 0,
                "completed_ids": ["instance-1"],
                "incomplete_ids": ["instance-2"],
                "empty_patch_ids": [],
                "submitted_ids": ["instance-1"],
                "resolved_ids": ["instance-1"],
                "unresolved_ids": [],
                "error_ids": [],
            }
            (cwd / "m.run.json").write_text(json.dumps(report), encoding="utf-8")
    ...
    pair = await SweBenchVerifiedAdapter().run(
        make_target(), _ctx(tmp_path, limit=2, run_id="missing-prediction")
    )

    assert pair.status == "partial"
    assert pair.metrics["score"] == 0.5
    assert pair.metrics["n_total"] == 2
    assert pair.metrics["n_incomplete"] == 1
    assert pair.methodology["selected_instance_ids"] == list(selected_ids)
    assert pair.methodology["prediction_instance_ids"] == ["instance-1"]
...
async def test_swebench_explicit_reruns_use_distinct_persistent_artifacts(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(swe_mod, "harness_missing", lambda: None)
    ...
    ctx = _ctx(tmp_path, limit=1, run_id="explicit-rerun", rerun=True)

    first = await adapter.run(make_target(), ctx)
    second = await adapter.run(make_target(), ctx)

    first_root = Path(first.methodology["artifacts"]["root"])
    second_root = Path(second.methodology["artifacts"]["root"])
...
async def test_swebench_subprocess_failure_is_stage_specific(
    ...
    artifacts = pair.methodology["artifacts"]
    assert Path(artifacts["root"]).is_dir()
    stderr_path = Path(artifacts["stage_logs"][f"{failing_stage}_stderr"])
    assert stderr_path.read_text(encoding="utf-8") == "stage exploded"
