...
def test_tiered_swebench_pro_command_is_full_dataset(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    benchmark = _load(EXAMPLE / "benchmark.py", "tiered_example_swebench_pro")
    observed: list[str] = []
    ...
    monkeypatch.setattr(benchmark, "_run", fake_run)
    monkeypatch.setattr(
        benchmark, "_validate_swebench_pro", lambda _path, **_kwargs: 0
    )

    assert benchmark.swebench_pro(tmp_path) == 0
    ...
    assert _option(observed, "--concurrency") == "4"
    assert _option(observed, "--max-output-tokens") == "32768"
    assert _option(observed, "--cache-dir") == str(benchmark.AGENTIC_CACHE)
    assert "--limit" not in observed


def test_tiered_swebench_pro_validator_requires_all_731_items(tmp_path: Path) -> None:
    benchmark = _load(EXAMPLE / "benchmark.py", "tiered_example_swebench_validator")
    run = tmp_path / "swebench-pro"
    result_dir = run / "swe-bench-pro--suite"
    result_dir.mkdir(parents=True)
    model = "kairyu-auto-max"
    task_count = benchmark.SWE_BENCH_PRO_TASKS
    scoreboard = {
        "cells": {
            "swe-bench-pro": {
                model: {
    ...
    result = {
        ...
        "items": [
            {"status": "completed", "score": 0.0, "error": None}
    ...
    assert (
        benchmark._validate_swebench_pro(
            tmp_path,
            run_id="swebench-pro",
            model=model,
