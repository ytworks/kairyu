...
from verification.l1.correctness import kv_answer_equivalence_bench as wrapper
from verification.l1.correctness import kv_equivalence as gate
...
def test_f4b_parent_rejects_failed_quality_owner_result(
    ...
    owner = ModuleType("verification.fleet.correctness.agentic_kv_tier_f4b_bench")
    ...
    import verification.fleet.correctness as verification_package
...
sys.argv = [{str(ROOT / "verification/l1/correctness/kv_answer_equivalence_bench.py")!r}, '--help']
try:
...
def test_direct_cli_reexecs_isolated_before_pythonpath_stdlib_shadow(
    ...
    completed = subprocess.run(
        [
            sys.executable,
            str(ROOT / "verification/l1/correctness/kv_answer_equivalence_bench.py"),
            "--help",
        ],
        cwd=ROOT,
        env=environment,
...
def test_module_form_is_not_an_evidence_invocation() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "verification.l1.correctness.kv_answer_equivalence_bench",
            "verify",
            "--artifact",
            "unused",
        ],
...
import runpy
import sys
from verification.l1.correctness import kv_equivalence
kv_equivalence.__file__ = sys.executable
runpy.run_path(
    {str(ROOT / "verification/l1/correctness/kv_answer_equivalence_bench.py")!r},
...
"""
    ...
    assert completed.returncode != 0
    assert (
        "refusing non-checkout verification.l1.correctness.kv_equivalence origin"
        in completed.stderr
    )
