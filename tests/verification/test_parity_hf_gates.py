...
def _decide(**overrides):
    from verification.l1.correctness.parity_hf import decide
...
def test_a_delta_at_the_bound_passes_and_just_over_it_fails():
    from verification.l1.correctness.parity_hf import _MAX_LOGPROB_DELTA
...
def test_cross_checkpoint_gate_binds_completeness_and_substantive_differences():
    from verification.l1.correctness.parity_hf import decide_cross_checkpoint
...
def test_a_matching_cache_is_accepted(tmp_path):
    from verification.l1.correctness import parity_hf
...
def test_reference_prompt_ids_match_kairyu_without_implicit_bos():
    from verification.l1.correctness.parity_hf import _prompt_ids
...
def test_cross_checkpoint_reference_accepts_same_quantized_model() -> None:
    from verification.l1.correctness.parity_hf import _validate_cross_checkpoint_reference
...
def test_teacher_forced_batches_never_mix_nested_prefixes():
    from verification.l1.correctness.parity_hf import _teacher_forced_waves
...
def test_different_weights_are_not_the_same_checkpoint(two_checkpoints):
    from verification.l1.correctness.parity_hf import _provenance
