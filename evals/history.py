...
_SAMPLING_EVALUATION_RESOURCE = ("evals", "sampling.py")
_STRUCTURED_CONFORMANCE_FIELD = "structured_conformance"
_STRUCTURED_EVALUATION_RESOURCES = frozenset(
    {
        ("evals.adapters", "structured_output.py"),
        ("evals", "structured.py"),
        ("evals.fixtures", "structured-output.jsonl"),
    }
)
...
def _validate_evaluation_resources(value: object, *, adapter: str) -> None:
    ...
    if not any(
        package.startswith("evals.adapters") and resource != "base.py"
...
def _pair_result_binding(
    ...
    *,
    ...
    from evals.aggregate import _binary_confidence_interval
...
def _recomputed_sampling_sensitivity(pair: object, *, benchmark: str) -> dict | None:
    ...
    from evals.adapters import all_adapters
    from evals.aggregate import _sampling_sensitivity
    from evals.types import PairResult
...
def _recomputed_structured_conformance(pair: object, *, benchmark: str) -> dict | None:
    if benchmark != "structured-output":
        return None
    from evals.structured import structured_summary
    from evals.types import PairResult
...
def _validate_scoreboard(
    ...
    *,
    ...
    from evals.adapters import all_adapters
    from evals.adapters.base import GenerativeAdapter
    ...
    from evals.types import sampling_configuration_error
