"""Fail-closed comparisons between immutable eval runs.

This module contains only generic cross-run comparison for checkout-only evals.
"""
...
def build_run_comparison(baseline: dict, candidate: dict) -> dict:
    """Build a fail-closed ``BASE -> CANDIDATE`` eval score comparison.
...
def _run_cell_text(values: dict) -> str:
    ...
    if values["status"] == "completed" and denominator is not None:
        ...
        from evals.aggregate import _stored_wilson_bounds
...
def render_run_comparison_markdown(comparison: dict) -> str:
    ...
    lines = [
        f"# Eval run comparison — {baseline['run_id']} → {candidate['run_id']}",
        "",
