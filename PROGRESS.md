...
| G5 — Fleet scale (elasticity, KV-aware routing, P/D pools, tiering, tenancy) | Goal defined (`docs/goals/g5-fleet-scale.md`); amends m7 D2 (k8s as machine layer), m5 D4/m7 D6 (prefix-aware placement), m6 D1 staticness, ClusterSpec cap, m7 D8 (OTel). F1a now has production EndpointSlice discovery, one shared TLS context with origin-local outbound transports, equivalent-contract validation grouping, deferred audit encoding, and a replayable direct-NodePort kind churn gate. The latest complete 200-replica/ten-epoch run served 33,000/33,000 requests with zero errors, 3.071 ms overall placement p99, and all churn-window p99 values at 2.705–4.083 ms. It remained fail-closed because one 1 Hz raw EndpointSlice bracket was 1.249 s; a concurrent 250 ms endpoint-only observer now removes that evidence race, and a clean formal rerun is pending. |
