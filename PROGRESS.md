...
### 2026-09-05 — [amendment] DTO-D16: automatic repair ensemble in existing Example
- What: add a sixth judge choice and repair DAG while preserving the original
  five routes and all framework code; Qwen thinking stays medium (DTO-D14).
  Update Example readiness and trace gates, with CPU configuration/HTTP tests.
- Why: compare repair causes and test evidence with bounded role output; keep
  inexpensive direct routing and the original general ensemble available.
- Refs: `docs/design/example-dual-track-orchestration.md` DTO-D16;
  Example `SWE-EVIDENCE.md`; 8-GPU score/cost/tool validation remains pending.
