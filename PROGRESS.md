...
| G4 — MoE engine (fused experts, EP, MTP, NVFP4, MLA) | **G4.1 M-A1 implementation and evidence in progress** (`docs/design/g4-mid-moe.md`, reviewed APPROVE-WITH-AMENDMENTS). The pinned 134 GB Qwen3-235B NVFP4 checkpoint loads expert-sharded on real EP2/EP4. The first complete 64×16 formal EP2 comparison failed unchanged acceptance criteria at 917/1,024 exact selected tokens with 83 substantive disagreements, despite exact reference teacher chains and prefix handoff. A position-0 FlashInfer-versus-torch attention A/B did not approach TensorRT-LLM, while checkpoint/runtime inspection found two concrete MoE math differences: Kairyu used per-expert activation scales instead of the fused backend's layer-global FC1/FC2 maxima, and rounded FP32 router weights before final combine. Both paths now implement the reference scale/finalization policy with focused CPU coverage; the real EP2 rerun is the active closure step and formal EP4 remains stopped until EP2 passes. M-A2/M-A3 remain pending and E-KV keeps BF16 KV fail-closed. |
