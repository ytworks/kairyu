...
| G4 — MoE engine (fused experts, EP, MTP, NVFP4, MLA) | **G4.1 M-A1 implementation has advanced; the formal gate remains open** (`docs/design/g4-mid-moe.md`, reviewed APPROVE-WITH-AMENDMENTS). The pinned 134 GB Qwen3-235B NVFP4 checkpoint loads rank-owned routed experts on real EP2/EP4. The retained complete EP2 comparison remains a valid FAIL at 917/1,024 exact tokens with 83 substantive disagreements. The current implementation now matches the reference's stable selected-only routing, layer-global NVFP4 scales, rank-local BF16 MoE finalize, fused FlashInfer CUTLASS local-expert path, and row-parallel BF16 attention-output boundary. The bounded EP2 position-0 sequence improved from 54/64 exact with 9 substantive disagreements before fused compute, to 57/64 with 6 after fused compute, and to 60/64 with 3 after row-parallel `o_proj`. Because the unchanged zero-substantive gate is already impossible for that run, another 1,024-position capture is deliberately not claimed or repeated; the best implementation is being integrated as a progress PR that references rather than closes issue #166. A diagnostic real-checkpoint EP4 one-token forward now passes on all four ranks: every rank reports 94/94 fused-MoE blocks, 94/94 row-parallel attention-output blocks, its 32-expert contiguous ownership, local K=2,048, and zero unresolved meta state. Review fixed stale prepared-operand/device/scale ownership and complete-forward markers. It leaves two measured P2 memory optimizations in #166: canonical plus swizzled scales add 6.609375/3.3046875 GiB per EP2/EP4 rank, and the validated `o_proj` loader's maximum temporary allocation above final residency is 854/1,273 MiB. The formal EP4 1,024-position capture remains stopped until EP2 can pass. M-A2/M-A3 remain pending and E-KV keeps BF16 KV fail-closed. |
