...
| G5 — Fleet scale (elasticity, KV-aware routing, P/D pools, tiering, tenancy) | Goal defined (`docs/goals/g5-fleet-scale.md`); amends m7 D2 (k8s as machine layer), m5 D4/m7 D6 (prefix-aware placement), m6 D1 staticness, ClusterSpec cap, m7 D8 (OTel). **F1a complete:** production EndpointSlice discovery, origin-local outbound transports, equivalent-contract validation grouping, deferred audit encoding, and the replayable direct-NodePort kind gate passed its clean-commit 200-replica/ten-epoch formal run. It served 33,000/33,000 requests with zero 5xx, 429, transport failures, or unsent arrivals; overall placement p99 was 2.966 ms and every churn-window p99 was 2.680–4.082 ms. All ten concurrent 250 ms withdrawal observers replayed consistently and the maximum old-UID withdrawal was 2.510 s under the fixed five-second bound. |
