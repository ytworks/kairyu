...
| G4 — MoE engine (fused experts, EP, MTP, NVFP4, MLA) | **M-A1 retains its formal FAIL; M-A2 is complete; M-A3 has one retained formal FAIL and an optimized candidate awaiting remeasurement** (`docs/design/g4-mid-moe.md`, reviewed APPROVE-WITH-AMENDMENTS). The pinned Qwen3-235B NVFP4 implementation remains the measured best fused path, while the M-A1 result stays 917/1,024 exact with 83 substantive disagreements and is not reclassified. The clean-commit real M-A2 EP4 run passed all 12 binding checks and retained 88.143382% logical prefix reuse with exact all-rank allocation/page receipts. The first complete M-A3 ten-generation matrix retained all 4,630 raw rows and passed every provenance/request/graph check, but measured only 0.783818× SGLang completion tok/s/GPU and 1.352633× SGLang TTFT p99, so the gate remains FAIL. The next candidate keeps configured depth 5 for pure decode, caps unresolved admission/prefill work at two forwards, stops fill when producer work arrives, and carries fast-greedy rank status in the existing NCCL token packet instead of a final Gloo gather. Rank status now copies asynchronously beside public tokens and is validated symmetrically one control boundary later, avoiding an eager CUDA-to-host synchronization. CPU protocol coverage and the focused real-CUDA sidecar test pass, but the candidate has not yet been workload-measured. E-KV keeps BF16 KV fail-closed. |
