...
| G4 — MoE engine (fused experts, EP, MTP, NVFP4, MLA) | **G4.1 M-A1 implementation and evidence in progress** (`docs/design/g4-mid-moe.md`, reviewed APPROVE-WITH-AMENDMENTS). The pinned 134 GB Qwen3-235B NVFP4 checkpoint now loads expert-sharded on real EP2/EP4 and both Kairyu configurations produced the same one-prompt token/logprob/top-64 result. The pinned TensorRT-LLM 1.2.1 TP2/EP2 and TP4/EP4 references selected that same token within the fixed logprob tolerance. The complete 64×16, EP2/EP4 start/end-hashed formal artifact remains the active closure step; M-A2/M-A3 remain pending and E-KV keeps BF16 KV fail-closed. |
