...
| G4 — MoE engine (fused experts, EP, MTP, NVFP4, MLA) | **M-A1 retains its formal FAIL; M-A2 is implemented and ready for its independent real EP4 run** (`docs/design/g4-mid-moe.md`, reviewed APPROVE-WITH-AMENDMENTS). The pinned Qwen3-235B NVFP4 implementation remains the measured best fused path, while the M-A1 result stays 917/1,024 exact with 83 substantive disagreements and is not reclassified. Production `KairyuBackend` now exposes the narrow EP2/EP4 replicated-attention envelope, rejects TP composition and unsupported modes, uses topology-neutral distributed readiness/shutdown, and reports EP without mislabelling it as TP locally or through a gateway. One opt-in bounded recorder retains each rank's first-prefill prompt/cache/page view without breaking collective order on a diagnostic failure; the logical rate is counted once from the driver engine. The M-A2 operator fixes EP4 at 4,129 BF16 KV pages and the A7-lineage 64-session × 8-turn, 512-request trace, retains exact raw radix events and all-rank/source/checkpoint/container/GPU/kernel evidence, and supports manifest verification plus raw-only replay. Its real hardware artifact has not yet been captured. M-A3 remains pending and E-KV keeps BF16 KV fail-closed. |
