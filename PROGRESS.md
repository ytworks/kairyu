...
- The tiered RTX PRO example is now the V41C six-plus-two implementation candidate: V4.1 TP2/attention-DP3/EP6 with CPU Engram offload and two unchanged Qwen TP1 replicas; native five-profile routing and the complete Requirement/four-policy/five-candidate/review/synthesis/DeepSeek-audit DAG. Required roles, exact choice provenance, per-choice audit and explicit step allowances are implemented; the public head is disabled or bounded when necessary to preserve a required publisher. The verification harness requires real serial judge execution and fresh completed native baselines for mandatory primary rows, with no historical fallback. CPU contract/example checks pass; full-model startup and live gates await the requested downtime approval. The 32-head quantized-arithmetic gate passes 16/16 at unchanged tolerance; original floating-reference failures remain recorded. Historical DTO measurements remain in the example's MEASUREMENTS.md and do not attest these bytes.
