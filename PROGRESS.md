...
| G4 — MoE engine (fused experts, EP, MTP, NVFP4, MLA) | **G4.1 M-A1 implementation and evidence in progress** (`docs/design/g4-mid-moe.md`, reviewed APPROVE-WITH-AMENDMENTS). The pinned 134 GB Qwen3-235B NVFP4 checkpoint now loads expert-sharded on real EP2/EP4 and both Kairyu configurations produced the same one-prompt token/logprob/top-64 result. A full TP2/EP2 diagnostic showed that TensorRT-LLM 1.2.1's retained-KV decode and fresh-full-prefix paths agree on only 933/1,024 positions, with a 6.125-nat worst deficit, so the binding canonical is now the autoregressive fresh-prefill reference rollout and ordinary free runs are diagnostic only. The complete 64×16 EP2/EP4 same-prefix, start/end-hashed formal artifact remains the active closure step; M-A2/M-A3 remain pending and E-KV keeps BF16 KV fail-closed. |
