...
| G4 — MoE engine (fused experts, EP, MTP, NVFP4, MLA) | **M-A1 retains its formal FAIL; M-A2 is complete** (`docs/design/g4-mid-moe.md`, reviewed APPROVE-WITH-AMENDMENTS). The pinned Qwen3-235B NVFP4 implementation remains the measured best fused path, while the M-A1 result stays 917/1,024 exact with 83 substantive disagreements and is not reclassified. Production `KairyuBackend` now exposes the narrow EP2/EP4 replicated-attention envelope and complete topology reporting. The clean-commit real M-A2 EP4 run passed all 12 binding checks: 512/512 requests, 491,008 cached / 557,056 prompt tokens (88.143382%), identical totals and page identities on all four ranks, 512 exact raw cache events, and 4,128 retained blocks. Manifest verification and raw-only replay pass for `bench/results/g4-ma2-ep-kv-qwen3-235b-rtxpro6000-2026-08-02/`. M-A3 remains pending and E-KV keeps BF16 KV fail-closed. |
