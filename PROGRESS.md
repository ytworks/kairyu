...
- Why: repeated samples cannot establish a hard publication guarantee while existing FAIL/exhaustion publication remains; the user also ended further testing/refinement. Preserve earlier 9/9 + 128/128 on `20b800e4…`, standard-launch 2/3 image FAIL on `be39136a…`, and incomplete final recheck on `85f6e652…` without reattributing results or claiming closure.
- Refs: tiered example `measurements/20260908-final-verification-stopped.json`, `MEASUREMENTS.md`; PR #595. No merge, framework change, or second-machine test.
