...
| G4 — MoE engine (fused experts, EP, MTP, NVFP4, MLA) | **M-A1 retains its formal FAIL; M-A2 is complete; M-A3 retains one formal FAIL while its optimized candidate has passed a live same-trace diagnostic** (`docs/design/g4-mid-moe.md`, reviewed APPROVE-WITH-AMENDMENTS). The pinned Qwen3-235B NVFP4 implementation remains the measured best fused path, while the M-A1 result stays 917/1,024 exact with 83 substantive disagreements and is not reclassified. The clean-commit real M-A2 EP4 run passed all 12 binding checks and retained 88.143382% logical prefix reuse with exact all-rank allocation/page receipts. The first complete M-A3 matrix retained all 4,630 raw rows and every non-performance binding but measured only 0.783818× SGLang throughput and 1.352633× SGLang TTFT p99. The optimized candidate preserves depth 5, removes the per-replay CUDA-event status wait with an overlapped fixed Gloo scalar, and replaces FlashInfer's stock replay plan with device-only metadata packing plus `fast_decode_plan`. A pre-commit 128×128 live diagnostic completed 128/128 requests at 571.542867 tok/s/GPU and 1.025656-second TTFT p99 with seven fixed captures and zero eager fallback, above the retained SGLang 449.965–481.865 tok/s/GPU range. A new clean ten-generation matrix is still required before the formal FAIL can change. E-KV keeps BF16 KV fail-closed. |
