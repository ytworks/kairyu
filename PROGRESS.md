...
- The example surface includes the measured 8 x RTX PRO 6000 DeepSeek deployment and a runtime-validated one-GPU Qwen3.6 FP8 example; both route Open WebUI through Kairyu L3 to pinned vLLM L1. Qwen's stable MTP-3 configuration is selected after its complete 8K serving matrix and an MTP-5 failure comparison; the 20-row accuracy gate remains in progress, with all persistent data and compilation caches on NVMe
