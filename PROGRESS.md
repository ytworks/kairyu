...
G5 F2d remains pre-results and in progress. A33 withdraws the
selection-biased chosen-action-agreement tuner, normalizes the identifiable
policy parameter to `λ=β/α` with `α=1`, and fixes `λ=0.25` as the baseline.
Every candidate must replay complete training episodes from the same frozen
initial cache/background-load state over families disjoint from held-out
evaluation; the training winner is frozen before deterministic virtual-time
held-out replay from that state. Closure requires strict held-out mean-TTFT
improvement and complete, zero-failure, balanced-work, exactly joined,
independently replayable evidence. Arm execution order is neither binding nor
diagnostic. No F2d result or closure is claimed yet.
