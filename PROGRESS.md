...
| G4 — MoE engine (fused experts, EP, MTP, NVFP4, MLA) | **M-A1 retains its formal FAIL; M-A2 is complete; the M-A3 candidate and operator are implemented with formal evidence pending** (`docs/design/g4-mid-moe.md`, reviewed APPROVE-WITH-AMENDMENTS). The pinned Qwen3-235B NVFP4 implementation remains the measured best fused path, while the M-A1 result stays 917/1,024 exact with 83 substantive disagreements and is not reclassified. The clean-commit real M-A2 EP4 run passed all 12 binding checks and retained 88.143382% logical prefix reuse with exact all-rank allocation/page receipts. M-A3 adds bounded TP1/attention-DP4/EP4 request ownership, grouped direct NCCL, packed QKV, coordinated CUDA-graph decode, a fixed pipeline-depth-5 production server, and a fail-closed fresh-server SGLang v0.5.16 comparison operator. Real-checkpoint load/graph smoke and the depth diagnostic pass, but neither is substituted for the still-pending four-pair formal verdict. E-KV keeps BF16 KV fail-closed. |
...
- What: implemented the bounded Qwen3-235B NVFP4 TP1/attention-DP4/EP4 production path with request-owned attention, KV, sampling, and token packets; grouped direct-NCCL all-gather/reduce-scatter; compatible packed-QKV projection; and an all-rank coordinated CUDA-graph decision. A real-checkpoint four-GPU smoke proved direct NCCL plus capture/replay with no fallback. A same-checkpoint diagnostic measured pipeline depth 5 at 1,971.873 aggregate completion tok/s versus 1,697.000 for depth 1, so the formal Kairyu arm is fixed to depth 5. The fail-closed operator pins SGLang v0.5.16, exact checkpoint/runtime/container provenance, a 128-request ShareGPT trace, seven graph buckets, four fresh-server pairs, exact ratio medians, and raw-only replay; the formal verdict remains pending.
