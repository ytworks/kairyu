...
from verification.product.workloads.coding import (
    CodeItem,
    CodeRequest,
    build_request,
    extract_code_block,
...
from verification.targets import normalize_base_url
    ...
    with data_path.open(encoding="utf-8") as source:
        for line in source:
            ...
            if item_id in wanted:
                found[item_id] = CodeItem(id=item_id, payload=row)
...
def _request_for(
    ...
    request = build_request(item, max_tokens=max_tokens)
...
async def measure_quality(
    ...
    *,
    ...
    request = _request_for(item, max_tokens)
