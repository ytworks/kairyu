...
Run:
  uv run python verification/l1/correctness/parity_tp.py --tp 1,2 --num-prompts 64
  uv run python verification/l1/correctness/parity_tp.py \\
      --tp 1,2,4,8 --model-path /models/qwen3-32b \\
...
class _ToyRunner:
    ...
    def adopt_sampling_token_packet(self, scheduled, states, packet):
        for index, chunk in enumerate(scheduled):
            state = states[chunk.request_id]
            if not chunk.is_prefill or state.prefill_done:
                self._future_tokens.setdefault(chunk.request_id, {})[chunk.position] = int(
                    packet[index]
                )
...
def _make_runner(tp: int):
    ...
    return TPModelRunner(
        rank_runners=tuple(_ToyRunner(sampling_owner=rank == 0) for rank in range(tp)),
        comms=comms,
    )
...
def _checkpoint_provenance(model_path: str) -> dict:
    ...
    rollup = hashlib.sha256(json.dumps(weights, sort_keys=True).encode()).hexdigest()
...
def _compare(base: dict, candidate: dict) -> dict:
    ...
    return {
        ...
        "median_first_divergence": (
            sorted(first_divergence)[len(first_divergence) // 2] if first_divergence else None
        ),
...
def _real_runner(model_path: str, tp: int, num_pages: int, page_size: int):
    ...
    runner = PagedModelRunner(model, pool, sampler=Sampler(vocab_provider=lambda: vocab))
...
def main() -> int:
    ...
    for overlap in overlap_modes:
        base = _run(degrees[0], overlap, prompts, args.model_path, args.num_pages, args.page_size)
    ...
    continuations = {
        f"tp{degree}_overlap_{'on' if overlap else 'off'}": {
            request_id: list(token_ids) for request_id, token_ids in sorted(run.items())
        }
    ...
    all_exact = all(entry["exact_match"] == entry["prompts"] for entry in results.values())
    ...
    if args.model_path:
        ...
        # moved near-tie — `verification/l1/correctness/parity_hf.py` is what gates. Reported for
