...
if __name__ == "__main__":
    if __spec__ is not None and not _bootstrap_help:
        raise RuntimeError("A12 evidence commands require the direct checkout path, not python -m")
...
from verification.l1.correctness import batch_invariance as gate  # noqa: E402

for _module_name, _relative_origin in (
    ("kairyu", "kairyu/__init__.py"),
    (
        "verification.l1.correctness.batch_invariance",
        "verification/l1/correctness/batch_invariance.py",
    ),
):
...
_CHECKOUT_MODULE_PREFIXES = ("evidence", "kairyu", "verification")
...
def _live_checkpoint(model_path: Path) -> dict[str, object]:
    ...
    pinned_rollup = hashlib.sha256(json.dumps(weights, sort_keys=True).encode("utf-8")).hexdigest()
    checkpoint = {
        "model": gate.MODEL,
        ...
        "metadata_sha256": {name: gate.sha256_file(model_path / name) for name in metadata_names},
    ...
    _require(
        _same_json_value(checkpoint["metadata_sha256"], gate.EXPECTED_METADATA_SHA256)
...
def _assert_gpu_job_exclusive() -> None:
    ...
    _require(
        not foreign,
        "formal A12 run requires exclusive GPUs; active foreign jobs: " + ", ".join(foreign),
    )
...
def _json_safe(value: object, *, description: str = "runtime value") -> object:
    ...
    raise ValueError(f"{description} contains unsupported {type(value).__name__}")
                    ...
                    self._steps.append(
                        {
                            ...
                            "kind": (
                                ...
                                if scheduled and all(not chunk.is_prefill for chunk in scheduled)
                                else "mixed"
                            ),
...
def _collect_structure(
    ...
    return {
        "scheduler_steps": _json_safe(list(scheduler_steps), description="scheduler steps"),
        ...
        "decode_rank_stats": _json_safe(decode(reset=False), description="all-rank decode stats"),
    }
...
def _token_pieces(tokenizer: object, token_ids: Sequence[int]) -> list[str]:
    ...
    vocab = vocab_getter()
    _require(
        isinstance(vocab, (tuple, list)) and len(vocab) == gate.TOKENIZER_VOCAB_SIZE,
...
async def _request_observation(
    *,
    ...
    try:
        result = await backend.generate(request_factory(request_id, prompt_ids, sampling))
...
async def _capture_full_backend(
    *,
    ...
    _require(
        isinstance(pressure_prompts, list) and len(pressure_prompts) == gate.MAX_PRESSURE_REQUESTS,
...
def _runtime_record(
    *,
    ...
    expected = gate.FULL_RUNTIME_CONFIG if runtime_id == "full" else gate.CHUNK_RUNTIME_CONFIG
    loop = getattr(backend, "_loop", None)
    cache = getattr(backend, "_cache", None)
    ...
    _require(
        getattr(backend, "kv_cache_dtype_requested", None) == expected["kv_cache_dtype"],
    ...
    _require(
        tuple(getattr(graph, "configured_buckets", ())) == gate.CUDA_GRAPH_BUCKETS
    ...
    _require(
        getattr(graph_backend, "_warmup_iters", None) == expected["cuda_graph_warmup_iters"],
    ...
    layer_backends = [
        getattr(getattr(layer, "self_attn", None), "backend", None) for layer in layer_values
    ]
    _require(
    ...
    rows: list[dict[str, object]] = [
        {
            ...
            "source": {
                ...
                "stable_across_measurement": _same_json_value(source_start, source_end),
            },
            "checkpoint": {
                "measurement_start": dict(checkpoint_start),
                "measurement_end": dict(checkpoint_end),
                "stable_across_measurement": _same_json_value(checkpoint_start, checkpoint_end),
            },
    ...
    rows.append(
        {
            ...
            "requests": [
                _pressure_request_row(observation) for observation in pressure_observations
            ],
...
def _create_output_directory(path: Path) -> None:
    try:
        ...
        raise ValueError(f"refusing to overwrite existing A12 output directory: {path}") from error
    except OSError as error:
