...
the overlap pipeline ON and OFF, which ``verification/l1/correctness/parity_tp.py`` records and
``verification/l1/correctness/gate_a1.py`` assembles. A2 instead requires the fixed 64-prompt
Llama-3.3-70B FP8 measurement at TP=2/4/8;
``verification/l1/correctness/gate_a2.py`` assembles those three runs with the shared
HF reference and fails closed on incomplete evidence.
...
Run:
  uv run python verification/l1/correctness/parity_hf.py --model-path /models/llama-3.3-70b-fp8 \\
...
from verification.l1.correctness.parity_tp import _TEXT_PROMPTS, _code_provenance
...
def _checkpoint_weight_digests(root: Path) -> dict[str, str]:
    ...
    return {path.name: value for path, value in zip(weight_files, digests, strict=True)}
...
def _provenance(model_path: str, prompts: list[str], positions: int) -> dict:
    ...
    weight_id = hashlib.sha256(
        "".join(f"{name}:{value}\n" for name, value in sorted(weight_digests.items())).encode()
    ).hexdigest()[:16]
    ...
    # identical
    tokenizer_files = sorted(root.glob("tokenizer*.json")) + sorted(
        root.glob("special_tokens_map.json")
    )
    tokenizer_id = (
        hashlib.sha256(b"".join(f.read_bytes() for f in tokenizer_files)).hexdigest()[:16]
        if tokenizer_files
        else None
    )
...
def _validate_cross_checkpoint_reference(reference: dict, candidate: dict) -> None:
    ...
    if reference.get("checkpoint_weight_files") == candidate.get("checkpoint_weight_files"):
...
def _load_reference(path: Path, expected: dict) -> dict:
    ...
    if len(entries) != expected["num_prompts"]:
        raise SystemExit(f"{path} holds {len(entries)} prompts, expected {expected['num_prompts']}")
...
def _build_reference(
    ...
    for batch_start in range(0, len(prompts), batch_size):
        ...
        for batch_index, text in enumerate(texts):
            ...
            with torch.no_grad():
                full = torch.tensor([prompt_ids + continuation], device=input_device)
            ...
            for step in range(len(continuation)):
                ...
                values, indices = torch.topk(row, k=min(top_k, row.shape[-1]))
...
def decide(
    *,
    ...
    if rate < reference_self_agreement:
        return False, (
            f"agreement {rate:.6f} is below the reference's own {reference_self_agreement:.6f}"
        )
    if substantive:
        return False, f"{substantive} substantive disagreement(s)"
    if max_abs_delta > _MAX_LOGPROB_DELTA:
        return False, (f"max |logprob delta| {max_abs_delta} exceeds {_MAX_LOGPROB_DELTA}")
    if missing_samples:
        return False, (
    ...
    for position in range(positions):
        yield tuple(
            (
                name,
                position,
                tuple(entry["prompt_ids"]) + tuple(entry["continuation"][:position]),
            )
...
def _engine_next_tokens(
    ...
    from verification.l1.correctness.parity_tp import _real_runner
...
def main() -> int:
    ...
    if bool(args.checkpoint_repo) != bool(args.checkpoint_revision):
        parser.error("--checkpoint-repo and --checkpoint-revision must be provided together")
    if bool(args.reference_checkpoint_repo) != bool(args.reference_checkpoint_revision):
        parser.error(
    ...
    for name, entry in reference.items():
        ...
        hits = 0
        for position, (token, hf_token) in enumerate(zip(actual, expected, strict=False)):
    ...
    missing_predictions = sum(row["engine_token"] is None for row in raw_positions)
    ...
    tolerance_ok = not substantive and not missing_deltas and (raw_max_delta <= _MAX_LOGPROB_DELTA)
    mean_delta = round(sum(logprob_deltas) / len(logprob_deltas), 5) if logprob_deltas else 0.0
    ...
    config = {
        ...
        "hardware": {
            ...
            "runtime": (_gpu_runtime_provenance() if profile.arch == "cuda" else None),
        },
        "method": (
    ...
    payload = {
        ...
        "agreement": {
            ...
            "verdict": (
                "DIAGNOSTIC"
                if cross_checkpoint
                else ("PASS" if raw_rate >= noise_floor["raw_self_agreement_rate"] else "FAIL")
            ),
        ...
        "logprob_tolerance": {
            ...
            "verdict": ("DIAGNOSTIC" if cross_checkpoint else ("PASS" if tolerance_ok else "FAIL")),
            "binding": not cross_checkpoint,
        },
