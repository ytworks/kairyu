...
* one ``run`` row, the exact 64 ``verification.l1.correctness.parity_tp._TEXT_PROMPTS`` rows, four
...
Usage::
    ...
    python verification/l1/correctness/g4_ma1_qwen3_235b_nvfp4_bench.py run \
      --capture /results/engine-capture.jsonl \
      --output-dir /results/g4-ma1 --assert-gate
    python verification/l1/correctness/g4_ma1_qwen3_235b_nvfp4_bench.py verify \
      --artifact /results/g4-ma1 --assert-gate
    python verification/l1/correctness/g4_ma1_qwen3_235b_nvfp4_bench.py replay \
      --artifact /results/g4-ma1 --assert-gate
"""
...
from verification.l1.correctness.parity_tp import _TEXT_PROMPTS  # noqa: E402
...
EXPECTED_CONFIG_SHA256 = "2d002795e3d79a92e03114cceeccf08cf9f4ea02ac5cd594ca9bea1faeb2c0b8"
EXPECTED_INDEX_SHA256 = "593af4d5bae0b67f7edcd16195a27131c82cf5ae1f775b3ee68872be06bccfac"
EXPECTED_HF_QUANT_CONFIG_SHA256 = "6423e9dd81773597a783e1109ffeb8d8b6a30a85c0eb4e503524a4fdf5f7f98b"
...
PROMPTS_SHA256 = "a3acedd9a419e440b4a2ddb2bcd96e0a5f6151fcce48c8982a1608c36938633c"
SEED = 20_260_702
TOP_LOGPROBS = 64
WORLD_SIZES = (2, 4)
...
REFERENCE_RUNTIME: Mapping[str, object] = {
    ...
    "image_config_id": ("sha256:cb4d8af81c586a90235ae3739b6d4ddc5d8336f2174c8a1c6b573d2e13faf5d7"),
    "backend": "pytorch",
    "result_api": ("tensorrt_llm.LLM.RequestOutput.token_ids+logprobs"),
}
...
if len(PROMPTS) != PROMPT_COUNT or sha256_json(list(PROMPTS)) != PROMPTS_SHA256:
    raise RuntimeError(
        "verification.l1.correctness.parity_tp._TEXT_PROMPTS changed; amend the "
        "formal M-A1 contract instead of silently changing the fixed prompt population"
    )
...
def _session_config(arm: str) -> dict[str, object]:
    ...
    return {
        ...
        "kv_cache_policy": (
            {
                ...
                "free_gpu_memory_fraction_ceiling": (REFERENCE_KV_FREE_GPU_MEMORY_FRACTION),
            }
            if reference
            else {
                "kind": "fixed_pages",
        ...
        "moe_backend": (
            "CUTLASS" if reference else "flashinfer_cutlass_fused_moe_local_partial_all_reduce"
        ),
        "sampling_arguments": {
            "seed": SEED,
            "temperature": 0.0,
...
def formal_config() -> dict[str, object]:
    return {
        ...
        "prompts": {
            "source": "verification.l1.correctness.parity_tp._TEXT_PROMPTS",
            "count": PROMPT_COUNT,
...
def _environment_valid(value: object, *, arm: str) -> bool:
    ...
    return (
        ...
        and visibility["resolved_rank_uuid_order"] == [gpu["uuid"] for gpu in gpus]
        ...
        and topology["gpu_pci_bus_id_order"] == [gpu["pci_bus_id"] for gpu in gpus]
        and topology["gpu_numa_node_order"] == [gpu["numa_node"] for gpu in gpus]
    ...
    *,
    ...
    if _arm_stack(arm) == "reference":
        return (
            ...
            and observation["image_repo_digest"] == REFERENCE_RUNTIME["image_repo_digest"]
            and observation["image_config_id"] == REFERENCE_RUNTIME["image_config_id"]
        )
    if set(value) != {
        "name",
        "version",
...
def _kernel_rank_valid(value: object, *, arm: str, rank: int) -> bool:
    ...
    expected_projection_count = 4 * NUM_LAYERS + (3 * NUM_LAYERS * experts_per_rank)
    ...
    if reference:
        ...
        expected_load_info = {
            ...
            "owned_expert_indices_sha256": sha256_json(list(range(first_owned, last_owned + 1))),
            ...
            "rank_loaded_tensor_count": (
                LOADED_NON_EXPERT_TENSOR_COUNT + NUM_LAYERS * owned_count * 12
            ),
        ...
        evidence_valid = (
            ...
            == {
                ...
                "projection_inventory_sha256": value["projection_inventory_sha256"],
                "kernel_counts": {
    ...
    return (
        ...
        and value["native_nvfp4_projection_count"] == value["quantized_projection_count"]
        and value["fallback_projection_count"] == 0
        and isinstance(value["projection_inventory_sha256"], str)
        and _HEX64.fullmatch(value["projection_inventory_sha256"]) is not None
    ...
    *,
    ...
    if _arm_stack(arm) == "kairyu":
        return (
            ...
            and value["hf_quant_config_sha256"] == checkpoint.get("hf_quant_config_sha256")
        )
    if not isinstance(value, dict) or set(value) != {
        "kind",
        "source_model_path",
    ...
    return (
        ...
        and value["source_hf_quant_config_sha256"] == checkpoint.get("hf_quant_config_sha256")
        and value["runtime_hf_quant_config_sha256"] == EXPECTED_TRT_RUNTIME_HF_QUANT_CONFIG_SHA256
...
def _session_valid(value: object, *, arm: str, run: Mapping[str, object]) -> bool:
    ...
    return (
        ...
        and all(
            _kernel_rank_valid(kernel, arm=arm, rank=rank) for rank, kernel in enumerate(kernels)
        )
...
def _top_logprobs_valid(
    ...
    *,
    ...
    return (
        ...
        and all(left >= right for left, right in zip(logprobs, logprobs[1:], strict=False))
...
def _attention_output_shard_valid(
    ...
    *,
    ...
    return (
        ...
        and value["rank_slice_sha256"] == _attention_output_rank_slice_sha256(value)
        and all(
...
def _ownership_valid(value: object) -> bool:
    ...
    expected = list(range(rank * experts_per_rank, (rank + 1) * experts_per_rank))
    expected_owned_tensor_count = NUM_LAYERS * experts_per_rank * 12
    expected_rank_loaded_tensor_count = LOADED_NON_EXPERT_TENSOR_COUNT + expected_owned_tensor_count
    ...
    return (
        ...
        and value["fully_replicated_tensor_count"] == FULLY_REPLICATED_TENSOR_COUNT
        ...
        and value["rank_loaded_tensor_count"] == expected_rank_loaded_tensor_count
        ...
        and value["evidence_source"] == "checkpoint-index-and-build-contract"
...
def _failure_valid(value: object) -> bool:
    return (
        isinstance(value, dict)
        and set(value) == {"schema_version", "type", "arm", "error_type", "message"}
        and value["schema_version"] == SCHEMA_VERSION
...
def _score_cell(
    ...
    expected = {
        (f"p{prompt}", position) for prompt in range(PROMPT_COUNT) for position in range(POSITIONS)
    }
    ...
    if complete:
        for prompt_id, position in sorted(expected):
            ...
            if missing:
                missing_reciprocal.append({"prompt_id": prompt_id, "position": position})
            else:
            ...
            is_substantive = missing or ref_gap is None or ref_gap > TIE_GAP_NATS
    ...
    return {
        ...
        "max_selected_logprob_abs_delta": (max(selected_deltas) if selected_deltas else None),
...
def _free_run_diagnostics(
    ...
    for left_arm, right_arm, name in pairs:
        ...
        for prompt in range(PROMPT_COUNT):
            ...
            aligned_equal += sum(a == b for a, b in zip(left_tokens, right_tokens, strict=False))
            prefix = 0
            ...
            if isinstance(left_lps, list) and isinstance(right_lps, list):
                common_prefix_logprob_deltas.extend(
                    abs(float(a) - float(b))
                    for a, b in zip(left_lps[:prefix], right_lps[:prefix], strict=False)
                )
        diagnostics.append(
            {
                "name": name,
                ...
                "max_common_prefix_selected_logprob_abs_delta": (
                    max(common_prefix_logprob_deltas) if common_prefix_logprob_deltas else None
                ),
                "binding": False,
            }
        )
    ...
    *,
    ...
    for index, row in enumerate(rows):
        ...
        try:
            ...
            raise ValueError(f"raw row {index} is not finite canonical JSON") from exc
    ...
    checks["run_boundaries_ordered"] = (
        bool(rows) and rows[0].get("type") == "run" and rows[-1].get("type") == "run_end"
    )
    checks["formal_config_exact"] = isinstance(run, dict) and run.get("config") == formal_config()
    ...
    checks["no_runtime_failure"] = len(by_type["failure"]) == 0 and all(
        _failure_valid(row) for row in by_type["failure"]
    )
    checks["run_end_counts_exact"] = isinstance(run_end, dict) and run_end.get(
        "row_counts_before_end"
    ) == _row_counts([row for row in rows if row.get("type") != "run_end"])

    prompt_by_key, duplicate_prompts = _keyed_rows(by_type["prompt"], keys=("prompt_id",))
    checkpoint = run.get("checkpoint_start")
    tokenizer_rollup = (
        checkpoint.get("tokenizer_rollup_sha256") if isinstance(checkpoint, dict) else None
    )
    ...
    checks["exact_fixed_tokenized_prompts"] = (
        ...
        and all(row.get("tokenizer_rollup_sha256") == tokenizer_rollup for row in by_type["prompt"])
    )

    session_by_key, duplicate_sessions = _keyed_rows(by_type["session"], keys=("arm",))
    ...
    complete_sessions = (
        ...
        and all(_session_valid(session_by_key[(arm,)], arm=arm, run=run) for arm in ARMS)
    )
    ...
    if complete_sessions:
        ...
        sequential = all(
            _utc_timestamp(left["finished_at"]) <= _utc_timestamp(right["started_at"])
            for left, right in zip(ordered_sessions, ordered_sessions[1:], strict=False)
        )
    ...
    checks["bf16_kv_all_sessions"] = complete_sessions and all(
        session_by_key[(arm,)].get("config", {}).get("kv_cache_dtype") == "bfloat16" for arm in ARMS
    )
    checks["native_nvfp4_kernels_no_fallback"] = complete_sessions and all(
        all(
            _kernel_rank_valid(kernel, arm=arm, rank=rank)
            for rank, kernel in enumerate(session_by_key[(arm,)].get("kernel_ranks", []))
        )
        for arm in ARMS
    )
    ...
    ownership_by_key, duplicate_ownership = _keyed_rows(by_type["ownership"], keys=("arm", "rank"))
    expected_ownership_keys = {
        (KAIRYU_ARM[world_size], rank) for world_size in WORLD_SIZES for rank in range(world_size)
    }
    ownership_complete = (
    ...
    no_remote_or_meta = ownership_complete and all(
        row["remote_expert_tensor_reads"] == 0 and row["unresolved_meta_tensors"] == 0
    ...
    if ownership_complete:
        for world_size in WORLD_SIZES:
            rows_for_world = [
                ownership_by_key[(KAIRYU_ARM[world_size], rank)] for rank in range(world_size)
            ]
            for field in (
                "dense_replication_sha256",
                "router_replication_sha256",
            ...
            replicated_dense &= (
                len({row["rank_loaded_tensor_count"] for row in rows_for_world}) == 1
            )
            replicated_dense &= len({row["rank_loaded_bytes"] for row in rows_for_world}) == 1
            ...
            for field in (
                ...
                attention_partition &= len({row[field] for row in attention_rows}) == 1
            rank_slice_digests = [row["rank_slice_sha256"] for row in attention_rows]
            attention_partition &= len(set(rank_slice_digests)) == world_size
            attention_partition &= all(
                row["partition_sha256"] == sha256_json(rank_slice_digests) for row in attention_rows
            )
        for field in (
            "dense_replication_sha256",
            "router_replication_sha256",
            ...
            replicated_dense &= len({row[field] for row in by_type["ownership"]}) == 1
        for field in (
            "tensor_count",
            "tensor_names_sha256",
            "full_geometry_sha256",
            ...
            attention_partition &= (
                len({row["attention_output_shard"][field] for row in by_type["ownership"]}) == 1
            )
    checks["rank_invariant_replicated_and_load_geometry"] = bool(replicated_dense)
    checks["attention_output_shards_nonoverlapping_complete"] = bool(attention_partition)

    free_by_key, duplicate_free = _keyed_rows(by_type["free_run"], keys=("arm", "prompt_id"))
    expected_free_keys = {(arm, f"p{prompt}") for arm in ARMS for prompt in range(PROMPT_COUNT)}
    complete_free = (
        not duplicate_free
    ...
    prefixes_exact = complete_teacher and checks["exact_fixed_tokenized_prompts"]
    ...
    if prefixes_exact:
        for world_size in WORLD_SIZES:
            ...
            for prompt in range(PROMPT_COUNT):
                ...
                canonical = [
                    teacher_by_key[(ref_arm, prompt_id, position)]["canonical_token_id"]
                    for position in range(POSITIONS)
                ]
                for position in range(POSITIONS):
                    ...
                    reference_teacher_rollout_exact &= (
                        teacher_by_key[(ref_arm, prompt_id, position)]["selected_token_id"]
                        == expected_token
                    )
    checks["world_matched_teacher_prefixes_exact"] = bool(prefixes_exact)
    checks["reference_teacher_rollout_chain_exact"] = bool(reference_teacher_rollout_exact)
    ...
    for world_size in WORLD_SIZES:
        ...
        reference_rows = (
            {(key[1], key[2]): row for key, row in teacher_by_key.items() if key[0] == ref_arm}
            if complete_teacher
            else {}
        )
        candidate_rows = (
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    run_parser = subparsers.add_parser("run", help="ingest a completed strict engine-capture JSONL")
