...
from kairyu.bench.targets import normalize_base_url
...
def load_calibration_cases(
    ...
    if path is None:
        raw = _default_bytes()
        source = f"package:kairyu.bench.fixtures/{_DEFAULT_SET}"
    else:
        selected = Path(path)
...
def _load_bound_run(
    ...
    *,
    ...
    from kairyu.bench.runner import _run_identity
...
def _calibration_inputs_from_args(
    ...
    from kairyu.bench.config import build_judge_config
