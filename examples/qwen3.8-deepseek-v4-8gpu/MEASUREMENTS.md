...
> candidate passes a separate 16-case, 32-head quantized-arithmetic comparison;
> the original floating-reference fidelity gate remains FAIL. The final
> sections preserve both results and the same-fixture comparison. No full-model
> six-GPU startup, API gate or ensemble performance result is claimed. Historical
> V4 baseline fallbacks below have been removed from the new configuration.
