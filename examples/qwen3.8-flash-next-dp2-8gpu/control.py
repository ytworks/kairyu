#!/usr/bin/env python3
"""One-command lifecycle for Qwen3.8-Flash-Next-FP8 as two TP4 replicas on eight GPUs."""
...
def _storage_paths() -> dict[str, Path]:
    ...
    paths.update(
        {
            f"cache_{index}": environment / f"compile-cache/qwen-{index}"
...
def _preflight(env: dict[str, str]) -> None:
    ...
    for replica, group in enumerate(REPLICA_GPU_IDS):
        env[f"QWEN_{replica}_CPUSET"] = ",".join(
...
def up() -> None:
    ...
    print(
        ...
        f"({REPLICAS} TP4 replicas, four GPUs each; text + image input)"
    )
    print(
