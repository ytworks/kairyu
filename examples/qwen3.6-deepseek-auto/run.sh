...
if [ "$gpu_count" -ne 8 ]; then
  echo "This AUTO example pins DeepSeek-V4-Flash to GPUs 0-3 (TP4+EP) and Qwen3.6-27B to GPUs 4-7 (DP4); it requires 8 visible NVIDIA GPUs, found $gpu_count" >&2
  exit 1
fi

echo "GPUs 0-3: DeepSeek-V4-Flash-0731 (TP4, expert parallel)"
echo "GPUs 4-7: Qwen3.6-27B (DP4, one TP1 replica per GPU)"
# exported so the compose port mapping honours a custom PORT
PORT="${PORT:-8001}"
export PORT
echo "Serving qwen3.6-27b, deepseek-v4-flash-0731, kairyu-auto and kairyu-auto-max on http://127.0.0.1:$PORT/v1"
