#!/usr/bin/env bash
# Verify the live product through the pinned Open WebUI browser surface using
# the shared, unmodified scripts/webui_browser_smoke.mjs (tiered phase).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
BROWSER_IMAGE="kairyu-webui-browser:playwright-1.60.0"
WEBUI_BASE_URL="${WEBUI_BASE_URL:-http://127.0.0.1:3008}"
