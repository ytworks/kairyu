...
Status: **BLOCKED — every six-GPU DeepSeek candidate fails the finite-output gate on the unpatched pinned image (2026-09-13); CPU contracts pass; no public gate was run.**
...
**Conclusion (2026-09-13, stop rule of the approved plan, decision D2):** on the
unpatched pinned image every valid six-GPU DeepSeek topology that starts
produces corrupt output from the second request per engine onward — with
and without Engram CPU offload, at TP2 × DP3 and at TP1 × DP6. This matches
the closed PR #598 diagnosis (masked sparse-KV gathers read slot 0, whose
contents are non-finite on this host), which that PR only cured with a
FlashInfer kernel patch. The sibling TP8 example is not affected on this
host. This example therefore has **no serving configuration** under the
current constraints (no Kairyu change, no runtime patch, sibling image by
ID); the public gates were not run. All four candidate runs, the one L2
smoke request (`20260913T172857Z-l2-smoke`, published the Qwen head alone
while every DeepSeek stage failed), and the launch logs are retained.
