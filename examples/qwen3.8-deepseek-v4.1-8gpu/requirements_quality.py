...
_CONTRACT_CHECKS = frozenset(
    {
        "ensemble_executed",
        "one_successful_requirements_stage",
        "complete_checklist",
        "requirements_finish_within_cap",
        "audit_covers_checklist_ids",
        "audit_evidence_fields_present",
        "nonempty_final_answer",
        "checklist_not_published",
        "native_image_path_only",
        "complete_sse",
    }
)
...
def validate_result(case: dict, result: dict, *, requirement_cap: int) -> dict:
    ...
    if case["id"] == "headed-comparison":
        ...
        # The trace can establish absence of the removed bridge, not prove
        # model perception. Actual multimodal forwarding has CPU wire tests;
        # factual interpretation remains a separate GPU quality diagnostic.
        checks["native_image_path_only"] = not any(
            event.get("node") == "image_description" for event in events
        )
...
def replay_run(run_dir: Path, *, requirement_cap: int) -> dict:
    ...
    complete = (
        ...
        and all(r["status"] == "complete" for r in reports)
    )
    return {
        "schema_version": 2,
        ...
        "quality_diagnostics_passed": complete
        and all(r["quality_diagnostics"]["passed"] for r in reports),
        "reports": reports,
    }
...
def main() -> None:
    ...
    if args.replay_run:
        ...
        args.report.write_text(
            json.dumps(
                replay_run(args.replay_run, requirement_cap=args.requirement_cap),
                indent=2,
            )
            + "\n"
        )
