...
# profile must trace; both models receive images natively.
...
def _deepseek_port() -> str:
    return str(os.environ.get("DEEPSEEK_L1_PORT", SPEC["deepseek_l1_loopback_port"]))
...
def _ensure_environment(no_start: bool) -> dict:
    if not no_start:
        _run([sys.executable, str(HERE / "control.py"), "up"])
    spec = importlib.util.spec_from_file_location("v41_example_control", HERE / "control.py")
    control = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(control)
    attestation = control.runtime_attestation()
    if attestation["config_sha256"] != _served_config_sha256():
        raise SystemExit("control and verification disagree on configuration provenance")
    return attestation
...
def _route_report(samples: list[dict]) -> dict:
    ...
    for sample in samples:
        ...
        for stage in sample.get("trace", {}).get("stages", []):
            if stage.get("node") == _JUDGE_NODE and isinstance(stage.get("total_ms"), (int, float)):
    ...
    if complete and expected_route is not None:
        ...
        def stage_ok(sample: dict) -> bool:
            ...
            if judged_routes:
                ...
                if not any(
                    stage.get("node") == _JUDGE_NODE and stage.get("kind") == "classification"
                    for stage in stages
                ):
                    return False
...
def _serving(
    ...
    *,
    ...
    request_max_tokens = (
        int(config["auto_max_combined_max_tokens"]) if natural_completion else output_tokens
    )
    fixed_output_args = [] if natural_completion else ["--min-tokens", "32", "--ignore-eos"]
    public_tokenizer_args = (
        [
            "--public-tokenizer-url",
            f"http://127.0.0.1:{_deepseek_port()}/tokenize",
...
def serving_auto_max(run_dir: Path) -> int:
    ...
    return _serving(
        ...
        run_dir,
        tensor_parallel=SPEC["allocation"]["tier2"]["tensor_parallel_size"],
        replicas=1,
...
def _direct_dataset(source: Path, target: Path) -> None:
    """Exercise the shipped native non-thinking role on the same coding tasks.

    Plain L1 chat defaults to thinking-high in V4.1. Rendering the exact direct
    role lets the same middleware select non-thinking without changing the
    shared benchmark harness or temporarily reconfiguring the running server.
    """
    spec = yaml.safe_load((HERE / "auto-max.yaml").read_text())
    profile = next(p for p in spec["profiles"] if p["name"] == "deepseek_direct")
    template = profile["roles"][0]["prompt"]
    rows = json.loads(source.read_text())
    for row in rows:
        message = row["conversations"][0]
        message["value"] = template.format(query=message["value"])
    ...
    *,
    ...
    return _run(
        [
            ...
            str(max_tokens),
            "--temperature",
            "1.0" if model == "deepseek-v4.1-flash" else "0.0",
            "--seed",
            "0",
            "--timeout",
            "86400",
...
def serving_auto_max_coding(run_dir: Path) -> int:
    ...
    times the paired direct row. There is no borrowed TP8/old-model fallback. Samples the
    ...
    deepseek_url = f"http://127.0.0.1:{_deepseek_port()}/v1"
    ...
    for concurrency in config["concurrency"]:
        ...
        direct_dataset = run_dir / f"coding-direct-c{concurrency}.json"
        _direct_dataset(dataset, direct_dataset)
        ...
        if routes is not None and gated_ttft is None:
            ...
            continue
        if not isinstance(product_ttft, (int, float)) or not isinstance(direct_ttft, (int, float)):
            print(f"c{concurrency}: TTFT gate is missing measurements", file=sys.stderr)
            return 1
...
def requirements_quality(run_dir: Path) -> int:
    return _run(
        [
            str(ROOT / ".venv/bin/python"),
            str(HERE / "requirements_quality.py"),
            "--run-dir",
            str(run_dir),
            "--base-url",
...
def main() -> None:
    ...
    runtime = _ensure_environment(args.no_start)
    ...
    manifest = {
        ...
        "verification_files_sha256": {
            ...
            for name in (
                "verification.py",
                "requirements_quality.py",
                "requirements-quality-cases.json",
            )
        },
        "spec": SPEC,
    }
